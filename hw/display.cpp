...
{
    char buffer[1000];
    sprintf(buffer, "x86 Emulator - [%dx%d]", w, h);
    SDL_WM_SetCaption(buffer, "x86");
}
...
static int create_subwindow(int chrwidth, int chrheight, int x, int y)
{
    ...
    for (int i = 0; i < MAX_SUBWINDOWS; i++) {
        ...
        if (!sw->visible) {
            ...
            sw->textbuf = (uint8_t*)malloc(chrsize << 1);
            sw->pixbuf = (uint32_t*)malloc(chrsize * (CHAR_WIDTH * CHAR_HEIGHT) * SCREEN_BYTPP);
...
static void display_blit_menubar(void)
{
    ...
    ((void)menubar_overscan);
    display_update(0, h);
}

static void display_blit_statusbar(void)
{
    uint32_t *pix = (uint32_t*)surface->pixels, base = w * (h + CHAR_HEIGHT); // Screen size plus menu bar
...
static void realloc_bar_sizes(int width)
{
    ...
    if (menubar_width != new_menubar_width) {
        if (menubar_textbuffer)
            menubar_textbuffer = (uint8_t*)realloc(menubar_textbuffer, new_menubar_width << 1);
        else
            menubar_textbuffer = (uint8_t*)malloc(new_menubar_width << 1);

        // Menubar width
        if (statusbar_textbuffer)
            statusbar_textbuffer = (uint8_t*)realloc(statusbar_textbuffer, new_menubar_width << 1);
        else
            statusbar_textbuffer = (uint8_t*)malloc(new_menubar_width << 1);
    }
...
{
    struct menubar_listing* mbl = (menubar_listing*)malloc(sizeof(struct menubar_listing));
    mbl->nent = sz;
    mbl->entries = (char**)calloc(sizeof(char*), sz);
    mbl->offsets = (int*)calloc(sizeof(int), sz);
    mbl->cbs = (menu_click_cb*)calloc(sizeof(menu_click_cb), sz);
...
static void finalize_menubar(struct menubar_listing* mbl)
{
    ...
    sw.cw = width;
    sw.ch = height;
    uint8_t* textbuf = sw.textbuf = (uint8_t*)calloc(width * height, 2);
...
void display_update(int scanline_start, int scnalines)
{
    if (!resized)
        return;
    if ((w == 0) || (h == 0))
        return;
    if ((scanline_start + scnalines) > h)
        exit(-1);
    else
    {
        render_windows();
...
void display_init(void)
{
    ...
    menubar_listing* menulist = init_menubar_listing(5);
    int idx = 0;
...
{
    input_captured = y;
    SDL_WM_GrabInput((SDL_GrabMode)y);
    SDL_ShowCursor(SDL_TRUE ^ y);
    display_set_title();
}

void display_release_mouse()
{
    display_mouse_capture_update(0);
}

void display_handle_mouse_click(SDL_Event* event)
{
                ...
                if (event.button.y <= MENUBAR_HEIGHT)
                {
                    if (event.type == SDL_MOUSEBUTTONDOWN)
                    {
                        ...
                        for (int i = 0; i < MAX_MENUBAR_ITEMS; i++)
                        {
                            ...
                            if (dx <= (unsigned int)menubar[i].twidth)
                            {
                                indx = i;
                                break;
                            }
